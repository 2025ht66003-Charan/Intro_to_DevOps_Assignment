...
def init_db():
    ...
    cur.execute(
        ...
        "weight REAL, program TEXT, calories INTEGER)"
    )
    cur.execute(
...
def index():
    return jsonify(
        {
            "version": "2.0.1",
            "description": "Client persistence and weekly progress tracking",
        }
    )
...
def manage_clients():
    ...
    if request.method == "POST":
        ...
        cur.execute(
            "INSERT OR REPLACE INTO clients (name, age, weight, "
            "program, calories) VALUES (?, ?, ?, ?, ?)",
    ...
    return jsonify({
        "clients": [
            dict(zip(["name", "age", "weight", "program", "calories"], row))
            for row in rows
        ]
    }), 200
...
def manage_progress():
    ...
    if request.method == "POST":
        ...
        cur.execute(
            ...
            "VALUES (?, ?, ?)",
            (client_name, week, int(adherence)),
        )
        conn.commit()
        conn.close()
        return jsonify({"saved": payload}), 201
    ...
    return jsonify({
        "progress": [
            dict(zip(["client_name", "week", "adherence"], row))
            for row in rows
        ]
    }), 200
